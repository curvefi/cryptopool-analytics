...
if __name__ == '__main__':
    ...
    losses = np.array(simulator.losses)
    t = losses[:, 0]
    g = losses[:, 1]
    period_number = (t - t[0]) // (30 * 86400)
    end_of_periods = ((period_number[1:] - period_number[:-1]) == 1)
    t_end = np.hstack([t[:-1][end_of_periods], t[-1]])
    g_end = np.hstack([g[:-1][end_of_periods], g[-1]])
    t_start = np.hstack([t[0], t[:-1][end_of_periods]])
    g_start = np.hstack([g[0], g[:-1][end_of_periods]])

    apys = ((g_end / g_start) ** ((86400 * 365) / (t_end - t_start)) - 1) * 100

    import pylab
    pylab.plot((t_start - t[0]) / (30 * 86400) + 1, apys)
    pylab.xlabel('Month')
    pylab.ylabel('APY (%)')
    pylab.yscale('symlog')
    pylab.tight_layout()
