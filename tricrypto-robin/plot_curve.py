...
def f_profit_deriv(A, gamma):
    ...
    d_price = np.linspace(-0.5, 0.5, 6000)
...
def f_liq_density(A, gamma):
    c = PCurve(A, gamma, D, N)
    d_price = np.linspace(-0.3, 0.3, 6000)
    dens = []
    derivs = []
    for dp in d_price:
...
if __name__ == '__main__':
    import pylab
    # dp, dens = f_liq_density(int(2 * 3**3 * 10000), int(8.05e-5 * 1e18))
    dp, dens = f_liq_density(int(3 * (10 - 1) * 1e4), int(0.05 * 1e18))
